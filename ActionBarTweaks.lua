...
local function EnsureWindow()
  ...
  local w = CreateFrame("Frame", "ExtendedUIConfigFrame", UIParent, "BasicFrameTemplateWithInset")
  C.win = w
  w:SetSize(800, 600)
  w:SetPoint("CENTER")
  ...
  local procBtn = CreateFrame("Button", nil, w, "UIPanelButtonTemplate")
	procBtn:SetSize(140, 22)
	procBtn:SetPoint("TOPLEFT", w, "TOPLEFT", 350, -60)  -- Pas evt. y-waarde aan voor je layout!
	procBtn:SetText("Configure ProcStack")
	procBtn:SetScript("OnClick", function()
	  if ProcHelper and ProcHelper.ShowConfig then
		if w and w.Hide then w:Hide() end
		ProcHelper:ShowConfig()
	  else
		print("ProcHelper module niet geladen.")
	  end
	end)
  ScanBags()
  w:Hide()
